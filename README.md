...
- **通用聊天**：选择已配置模型直接对话，不注入业务 Prompt，不读取资料库或改写工作区。
...
首次启动会从 `.env.example` 创建本地 `.env`。至少配置一个模型 Provider API Key，例如 `QWEN_API_KEY`，对应模型才会出现在模型选择器中。`DEFAULT_CHAT_PROVIDER` 指定首选 Provider，默认模型取该 Provider 在 `*_CHAT_MODELS` 中配置的第一个可用模型；前端不单独维护模型默认值。
...
根目录 `docker-compose.yml` 只管理开发依赖，由 `./start.sh` 调用，不作为另一套应用启动入口。本地账号和工作区统一保存在 `./data`；MySQL、Qdrant 分别使用 `mysql_data`、`qdrant_data` named volumes。
