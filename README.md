...
Auto Reign 是本地优先的通用 Agent 聊天平台。用户在新会话发送第一条消息前选择 Agent；Agent 配置决定系统提示词、默认模型，以及是否获得 Agent Home 文件工具和 Knowledge 检索工具。聊天附件绑定一条用户消息，并在该消息仍进入有界历史窗口时作为不可信上下文使用，不会自动写入 Agent Home、资料库或 Knowledge Retriever。
...
- Agent Home 不会自动保存普通聊天、答题或讨论；只有用户明确要求长期保存、记录或更新时，模型才应调用文件工具。
- 可选 Knowledge Collection：用户或管理员显式上传 Document，每个 Collection 固定选择 Elasticsearch 或 Qdrant；Elasticsearch 支持 vector、BM25 keyword 和线性融合 hybrid，Qdrant 支持 vector。
...
同一个 Agent 可以同时绑定 Agent Home 和一个或多个 Knowledge Collection。主聊天 LLM 决定是否调用已经授予的工具；用户隔离、路径、ETag、Collection/Document 范围、Retriever filter、预算和持久化均由服务端确定性代码执行。
...
| Knowledge chunk | Elasticsearch 或 Qdrant | 可删除并从权威源重建，不保存 Agent Home、聊天附件或日志 |
...
平台当前不依赖 Redis、Kibana、Celery 或其他消息队列。Elasticsearch 和 Qdrant 只服务 Knowledge 投影，不保存请求日志或 Message 权威数据。Retriever 连接、认证和索引名由部署配置维护；普通用户创建资料库时选择 Retriever 类型，创建后不可修改，但仍可调整检索模式、Top K、阈值、hybrid 权重和分块配置。`GET /api/health/retrievers` 用于连接检查。

引入共享 Retriever 的 `20260720_0005` 迁移不兼容历史 Knowledge 配置或投影：历史 Collection 配置会重置为 Elasticsearch、vector 和当前检索默认值；所有历史 active Document 会 generation + 1 并重新进入 `queued`，从 ObjectStore source 重新解析和索引；重建完成前不会保留虚假的 `ready`。旧 Qdrant-only 投影不读取、不迁移，也不增加兼容层。
...
`./reset-data.sh --yes` 是显式破坏性本地操作，只清理本地 MySQL、Elasticsearch、Qdrant volume 和仓库内本地 runtime data。远端 S3/OSS 对象永不随该命令清空；应用也不会自动删除本地或远端用户数据。

## 运行与发布

| 模式 | 启动方式 | 用途 |
| --- | --- | --- |
| 开发模式 | `./start.sh` | MySQL、Elasticsearch、Qdrant 在 Docker 中运行，FastAPI、Next.js 在宿主机运行 |
