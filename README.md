...
Auto Reign 可以把 Markdown、TXT、PDF、DOCX、自由文本学习笔记和真实面试记录写入本地工作区，在 Qdrant 中索引可检索 chunk，以聊天式流程进行书面模拟面试，并在本地保存面试历史、题库、高频问题和复盘记忆。自由文本学习笔记会追加到每日 `inbox/YYYY-MM-DD.md`，再整理为面试短卡片；真实面试记录会保存到 `raw/`，并更新高频问题和复习状态。
