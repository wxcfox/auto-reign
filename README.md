...
Auto Reign 可以把 Markdown、TXT、PDF、DOCX、自由文本学习笔记和真实面试记录写入本地工作区，在 Qdrant 中索引可检索 chunk，以聊天式流程进行书面模拟面试，并在本地保存历史对话、题库、高频问题、复习状态和复盘报告。自由文本学习笔记会追加到每日 `inbox/YYYY-MM-DD.md`，再整理为面试短卡片；真实面试记录会保存到 `raw/`，并更新高频问题和复习状态。
...
- MySQL：工作区 artifact 投影、学习对话、面试和报告元数据
- Qdrant：索引后的 chunk 向量和检索 payload
- `DATA_DIR`：工作区来源文件、提取文本、生成的 Markdown、复盘报告、修订版本和本地工作数据
