...
`./reset-data.sh --yes` 是破坏性命令：它会停止本地 Auto Reign 进程，删除应用数据、MySQL 和 Qdrant Docker volume，并删除 `data/`、`.pids/`、`logs/` 等本地运行数据，包括 `DATA_DIR/users` 下的本地用户数据和旧版 `DATA_DIR/workspace`。它不会删除源代码、依赖目录或 `.env` 等本地配置文件。
...
持久化数据会保留在 `app_data`、`mysql_data` 和 `qdrant_data` 命名 volume 中。`docker compose down` 不删除这些 volume；只有显式执行 `./reset-data.sh --yes` 或 `docker compose down -v` 才会删除。

全容器模式的 `app_data` 与 `./start.sh` 源码模式使用的 `./data` 是两套独立数据，账号、工作区和 JWT 密钥不会在两种模式间自动共享。日常使用应固定选择一种启动模式，不要把切换启动方式当作同一份数据的重启操作。
...
面试创建、答题、追问、下一题和收尾只提供 `/stream` SSE mutation API，前端和测试使用同一条线上路径，不保留并行的非流式 LLM 实现。workspace 文件投影由上传、编辑、删除和归档等确定性应用流程维护，不暴露测试专用的投影重建 API。
