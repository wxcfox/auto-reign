...
认证采用本地用户名密码加 JWT。用户注册或登录成功后，后端签发 access token。JWT 签名密钥优先读取 `JWT_SECRET_KEY`；未配置时，后端在 `DATA_DIR/.secrets/jwt_secret` 生成当前安装专用的本地密钥。前端后续请求统一携带：
...
- `messages` 使用 `UNIQUE(conversation_id, sequence)` 保存会话内写入顺序。
