...
- 知识、题库、项目、真实面试、高频复盘和练习 Markdown artifact 从正文内容索引。新学习先取得经过结构化校验的整理结果，再由确定性本地化 renderer 生成短卡片，并按标题 slug 合并到同一个 `knowledge/<主题>.md`。中文卡片使用「我的理解 / 修正/补充 / 30 秒面试说法 / 易混点 / 追问」结构，英文卡片使用对应英文结构。学习对话本身用于侧边栏历史和继续追加，不作为长期知识资产，也不直接进入向量索引。
- 答差或缺失点会沉淀为 `questions/<题目>.md`，中文结构包含「考察点 / 标准回答 / 结合项目 / 常见追问 / 易错点 / 复习状态」，英文使用对应英文结构。
