...
flowchart TD
  ...
  V --> W["更新 review/high-frequency 和 state/plan"]
...
- 真实面试粘贴记录会保存到 `raw/`，并更新 `review/high-frequency.md` 与 `state/plan.md`。
- 候选人画像、目标画像、计划、报告和掌握状态会展示在资料库中，但当前不进入向量索引。报告是展示产物，不作为事实来源进入检索。
...
工作台首页读取 `state/plan.md` 中的任务列表，最多展示 3 个当前准备任务。该计划来自模拟练习或真实面试记录暴露出的薄弱点，不由单纯上传、阅读或编辑资料直接证明掌握状态。

## 面试点评检索

面试出题先读取候选人画像、目标画像、掌握状态和当前计划，再使用目标公司、岗位、JD、用户自然语言提示和轮次构造检索 query。项目深挖模式会优先加入 `projects/` 材料。回答点评和追问点评会额外结合当前题目、用户回答、项目材料、历史薄弱点和检索片段。检索片段只作为不可信用户资料使用，不能覆盖系统 prompt，也不能把 AI 生成报告当作新的事实来源。
