...
MySQL 和 Qdrant 只在 Compose 内部网络中可见。FastAPI 和 Next.js 只映射宿主机回环地址；安全组和 Nginx 不应公开 backend、frontend、MySQL 或 Qdrant 的内部端口。

生产 Compose 只有 MySQL、Qdrant、一次性 migrate、backend 和 frontend。当前不部署 Redis、Elasticsearch、Kibana、Celery 或独立 Knowledge worker container。
...
`/srv/auto-reign/data` 只是容器本地 runtime 目录，不是生产附件、Agent Home 或 Knowledge 的文件权威源。三类文件都保存在远端 S3-compatible ObjectStore。
...
同时填写 MySQL、ACR 和实际使用的模型或 Embedding Provider 配置。Secret 只保存在权限为 `0600` 的生产 env 或外部 Secret 管理系统中，不能提交仓库、写入前端或输出日志。

应用运行时的对象大小、上下文预算、Knowledge 检索、Worker、模型超时和工具轮次上限也应显式填写；这些配置由 `Settings` 统一读取，完整示例见 `deploy/auto-reign.env.example`。Compose 只注入容器路径、数据库 URL、Qdrant URL 和发布版本等部署拓扑值。
...
不要开放 backend/frontend loopback 端口、MySQL 或 Qdrant。安装并检查仓库 Nginx 配置：
...
证书由 Certbot 或现有证书管理系统维护。反向代理必须保留 SSE 所需的流式行为，并只通过同域 `/api` 暴露 FastAPI。
...
应用 stdout 默认一行一个 JSON object。每个 HTTP 响应包含 `X-Request-ID`：安全的调用方 ID 会保留，非法、空白、换行、Unicode 或超长值会替换为 UUID。该 ID 在完整请求生命周期中绑定，SSE duration 覆盖流结束或取消。
...
只有 `app`/`app.*` 的稳定 event 和 allowlist extra 可以透传。第三方 SDK 日志统一收敛为 `third_party_log`，只保留安全 logger、level 和 exception type。日志轮转由 Compose 的 Docker `json-file` 设置和宿主机日志平台负责；应用不部署 Elasticsearch，也不把 stdout 复制到 MySQL。

## 生成与工具审计

生成审计保存在 MySQL Assistant metadata：
...
每次部署前，`deploy/backup.sh` 备份 MySQL 和 `AUTO_REIGN_DATA_DIR`，并写入版本与校验信息。MySQL 备份包含用户、资源、Message、Document 状态和生成审计；本地 runtime archive 不包含远端 ObjectStore 权威文件。

Qdrant 是可重建投影，不在默认部署备份中。恢复时从 MySQL 当前 Document generation 与 ObjectStore 权威解析文本重新索引。
...
普通 `./reset-data.sh --yes` 只面向本地开发：它只识别仓库内 `DATA_DIR`、`OBJECT_STORE_LOCAL_ROOT` 和本地 Docker MySQL/Qdrant volumes。路径在仓库外会被拒绝。

该命令不读取 S3 配置，不构建远端 client，也不 list 或 purge S3/OSS。远端对象永远不是普通 reset 候选；生产清理不能复用该命令。

## ObjectStore orphan audit

`scripts/audit_object_orphans.py` 是显式、report-only 的诊断工具，不由应用启动、后台 worker、cron 或 reset 调用。它比较 MySQL 引用与 ObjectStore list snapshot，区分：
...
当前只能运行单个 FastAPI 进程，因为 SSE、取消、Knowledge worker 和 Agent Home 同 Key 串行化都依赖当前进程。没有跨进程协议前，不得增加 Uvicorn worker、backend replica 或独立 worker container。
...
只有出现明确的跨进程协调需求时才评估 Redis；它不能成为 Message 或文件的唯一权威。Elasticsearch 不是当前日志或审计依赖。未来迁移 Kubernetes 时可以继续使用相同 ACR 版本镜像和 Git Tag，但 Helm、Secret 和存储策略应由独立运维设计承担。
