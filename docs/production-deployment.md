...
MySQL、Elasticsearch 和 Qdrant 只在 Compose 内部网络中可见。FastAPI 和 Next.js 只映射宿主机回环地址；安全组和 Nginx 不应公开 backend、frontend、MySQL、Elasticsearch 或 Qdrant 的内部端口。

生产 Compose 只有 MySQL、Elasticsearch、Qdrant、一次性 migrate、backend 和 frontend。当前不部署 Redis、Kibana、Celery 或独立 Knowledge worker container。Elasticsearch 只是 Knowledge Retriever，不是日志栈。
...
应用运行时的对象大小、上下文预算、Knowledge 检索、Worker、模型超时和工具轮次上限也应显式填写；这些配置由 `Settings` 统一读取，完整示例见 `deploy/auto-reign.env.example`。Compose 只注入容器路径、数据库 URL、Retriever URL 和发布版本等部署拓扑值。
...
不要开放 backend/frontend loopback 端口、MySQL、Elasticsearch 或 Qdrant。安装并检查仓库 Nginx 配置：
...
3. 拉取明确版本镜像；
4. 启动 MySQL、Elasticsearch、Qdrant 并执行 Alembic migration；
...
普通 `./reset-data.sh --yes` 只面向本地开发：它只识别仓库内 `DATA_DIR`、`OBJECT_STORE_LOCAL_ROOT` 和本地 Docker MySQL/Elasticsearch/Qdrant volumes。路径在仓库外会被拒绝。
...
只有出现明确的跨进程协调需求时才评估 Redis；它不能成为 Message 或文件的唯一权威。Knowledge 使用的 Elasticsearch 不是日志或审计依赖。未来迁移 Kubernetes 时可以继续使用相同 ACR 版本镜像和 Git Tag，但 Helm、Secret 和存储策略应由独立运维设计承担。
