...
- `learning_sessions` 与 `learning_messages`：新学习的聊天式历史上下文。工作区 Markdown 仍是长期学习资产权威，学习对话只保存用户输入、整理结果和对应 artifact 引用。
...
- 历史会话：侧边栏通过统一对话接口合并面试和学习，列表只展示标题、最近内容和时间，不展示“已完成”或“处理中”这类状态标记。
