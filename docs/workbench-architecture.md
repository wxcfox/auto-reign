...
- `conversations` 与 `messages`：按用户隔离保存面试和学习的统一对话投影。`conversations.kind` 区分 `interview` 与 `learning`；历史会话重命名写入会话投影，删除历史只设置软删除标记，使其不再出现在统一历史或直接会话读取中。工作区 Markdown 仍是长期学习资产和练习证据的权威，学习对话只保存用户输入、整理结果和对应 artifact 引用；删除学习历史不得删除已写入工作区的资料和知识卡片。
