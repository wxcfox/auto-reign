...
| Knowledge Collection | 创建时确定且不可修改的 `retriever_type`；可编辑的 `retrieval_mode`、chunk、overlap、`top_k`、threshold 和 hybrid 权重 | 显式包含 Document；Agent 可绑定整库或精确 Document 子集 |
| Knowledge Document | Collection、owner、Retriever、对象 Key、状态、内容 hash、`index_generation` | 原文显式上传；解析和 Retriever 投影可重建 |
...
| Elasticsearch | 从 Knowledge 当前解析文本生成、可重建的原文 chunk、dense vector 和 BM25 投影 |
| Qdrant | 从 Knowledge 当前解析文本生成、可重建的原文 chunk dense vector 投影 |
...
Elasticsearch、Qdrant、stdout 日志和 Assistant audit metadata 都不是正文权威。两种 Retriever 都可以从 ObjectStore 与 MySQL 当前 generation 重建；平台不另外保存包含完整 Prompt、历史、附件、文件或 RAG chunk 的模型请求副本。
...
Home 初始化或读取失败返回稳定的 `workspace_unavailable`，不能回退旧模板。Home 文件不创建 Knowledge Document，不做 chunk/embedding，也永不查询或写入 Knowledge Retriever。
...
| Knowledge | 显式维护的只读参考 Document | `search_knowledge(query)`，直接原文或 Collection 配置的 Retriever chunk |

附件原文与解析文本保存在 ObjectStore，但不创建 Knowledge Document、不 chunk、不 embedding、不写 Retriever，也不自动写入 Home。附件只有在所属消息进入当前有界历史窗口时才会读取。
...
| Document 状态、generation 和 Retriever filter | 否 | 是 |
| Retriever、retrieval mode 与 hybrid 权重 | 否 | 从 Collection 固定配置读取 |
| 历史裁剪和 Token 预算 | 否 | 是 |
| 数据库、ObjectStore 与 Retriever 读写 | 否 | 是 |
| 文档解析、chunk 和索引发布 | 否 | 是；Embedding 模型只生成向量 |

LLM 永远不持有 DB Session、ObjectStore client、Retriever client 或 Secret。它只能提出经过 schema 校验的工具调用；所有权限、状态机、预算和持久化由应用代码执行。
...
- Elasticsearch 和 Qdrant 是平台共享 Knowledge Retriever，只用于当前 generation 的可重建投影、检索和清理；
- 当前不部署 Redis、Kibana、Celery 或独立消息队列。

只有确实需要跨进程流、取消、队列或锁时才设计 Redis 等协调设施；任何协调层只能保存可丢失、可重建的状态，不能成为 Message 或文件唯一权威。Knowledge 使用的 Elasticsearch 不是聊天日志或审计索引，应用不依赖日志索引才能运行。
...
当前 baseline 不兼容旧表、旧文件协议、旧 Retriever 配置与投影或旧 Prompt 分支；不双读、双写或保留兼容字段。共享 Retriever 迁移会把历史 Knowledge Collection 重置为新的 Elasticsearch 默认配置，把历史 active Knowledge Document 递增 generation 并重新排队，从 ObjectStore source 建立新投影，不允许没有对应索引的 Document 继续显示为 `ready`。Schema guard 检测到旧 revision、旧业务表或非空未版本化 schema 时拒绝启动，要求操作者显式处理。应用不会自动 DROP、迁移或删除本地及远端用户数据。
