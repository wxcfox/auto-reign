...
- 当前基线：[平台架构](../architecture/platform.md)
- 设计基线：Task/Subtask 历史权威、统一上下文预算、摘要压缩和 `messages_chain` 序列化契约

> 本文描述待实施的目标契约，不代表当前运行行为。当前实现只执行确定性历史裁剪与预算复核，运行事实以[平台架构](../architecture/platform.md)为准。

本文定义模型可见上下文治理的后续方案。MySQL 保存完整原始历史；治理只改变某次模型调用实际看到的消息，不删除或改写原始 User/Assistant Subtask。
